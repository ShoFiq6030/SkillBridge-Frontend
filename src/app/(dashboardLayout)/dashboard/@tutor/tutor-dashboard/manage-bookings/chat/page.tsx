    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? (
          ...
          filtered.map((room) => <ChatRoomCard key={room.id} room={room} isTutor={true} />)
        )}
      </div>
    </div>
  );
...
function PageHeader() {
  return (
    <div>
      <h1 className="text-3xl font-bold tracking-tight">Tutor Chats</h1>
      <p className="text-muted-foreground">
        View and communicate with Students about their bookings.
      </p>
    </div>
  );
}
